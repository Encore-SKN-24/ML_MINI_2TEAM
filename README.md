...
## 6. ℹ️ 인사이트 도출 
> Logistic Regression, Random Forest, XGBoost 모델을 비교 실험한 결과, 전세사기 위험 분석을 통해 충분히 분류 가능함을 확인했습니다. 특히 Random Forest와 XGBoost와 같은 트리 기반 앙상블 모델은 복잡한 변수 간 상호작용을 효과적으로 학습하여 선형 모델보다 우수한 성능을 보이는 것을 파악하였습니다. 그 중 특히, XGBoost 모델은 위험 매물 탐지 능력에서 가장 뛰어난 결과를 나타낸다는 점도 파악할 수 있었습니다. 이는 전세사기 예방 목적에서 ‘정확도’보다 ‘위험 사례를 놓치지 않는 탐지율’이 더 중요하다는 점을 시사하며, 결과적으로 트리 기반 앙상블 방식이 본 문제에 가장 적합한 모델임을 알 수 있었습니다.
...
## 8. 💭 한 줄 회고
### - 권민세: 
 전 프로젝트에서 수행했던 전처리 과정을 기반으로 Logistic Regression, Random Forest, XGBoost 등 다양한 머신러닝 모델을 적용해보며 회귀 모델에 대한 이해도를 높일 수 있었습니다. 변수 값을 정의하는 과정에서 y값에 포함될 데이터를 분류하는 데 혼란을 겪어 원하는 결과를 얻는 데 어려움이 있었지만, 기준을 다시 정리하며 문제를 해결할 수 있었습니다. 이 경험을 통해 앞으로는 모델링 단계에 앞서 데이터 분리와 타깃 변수 정의 과정을 더 신중하게 설계해야겠다고 느꼈습니다.
...
### - 황인규:
전처리 과정을 통해 데이터 정제의 중요성을 이해하고, 데이터 간 상관관계 분석을 통해 데이터 분석의 의미를 학습하였습니다. 또한 X 데이터와 y 정답 데이터를 어떻게 정의할지에 대해 많은 고민을 하며, 정하고자 하는 y 데이터의 근거를 찾아 타당하게 설정하는 과정을 경험하였습니다. 이를 바탕으로 분류와 회귀 모델을 구축하고 학습 및 평가 과정을 수행하며 머신러닝 모델을 이해하였고, 하이퍼파라미터 조정을 통해 모델 성능이 설정에 따라 변화하는 것도 확인할 수 있었습니다. 더 나아가 여러 모델을 비교한 결과 예측 성능이 가장 우수했던 RandomForest 모델을 중심으로 정밀도, 정확도, 재현율 등의 지표를 활용해 모델을 보다 정밀하게 평가하는 방법을 익혔습니다.
