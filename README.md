...
### - 황인규:
전처리 과정을 통해 데이터 정제의 중요성을 이해하고, 데이터 간 상관관계 분석을 통해 데이터 분석의 의미를 학습하였다. 또한 분류와 회귀 모델의 학습 및 평가 과정을 수행하며 머신러닝 모델을 구축하고 성능을 평가하는 방법을 익혔고, 하이퍼파라미터를 조정하면서 모델 성능이 어떻게 변화하는지도 확인할 수 있었다.
